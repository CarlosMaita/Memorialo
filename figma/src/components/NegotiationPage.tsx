...
function getStatusLabel(status: string): string {
  switch (status) {
    ...
    case 'pending_client': return 'Pendiente tu firma';
...
export function NegotiationPage({ contract, booking, user, onBack, chatApi }: NegotiationPageProps) {
  ...
  const conversationReference = useMemo(() => {
    const contractId = booking?.contractId ? String(booking.contractId).trim() : '';
    if (contractId) return { label: 'Contrato', code: contractId };
    const bookingId = conversation?.bookingId ? String(conversation.bookingId).trim() : '';
    if (bookingId) return { label: 'Reserva', code: bookingId };
    return null;
  }, [booking?.contractId, conversation?.bookingId]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const conv = await chatApi.ensureChatConversation({ bookingId: booking.id });
        if (!cancelled) {
  ...
  // ── Contract info ──────────────────────────────────────────────────────
  const contractStatus = contract?.status || booking?.status || '';
  const contractDate = contract?.terms?.date || booking?.date || '';
      ...
      <Card className="border-white/40  shadow-sm">
        <CardContent className="p-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h3 className="font-semibold text-[#1B2A47] text-sm">{providerName}</h3>
              {contract?.metadata?.providerBusinessName && (
                <p className="text-xs text-gray-500">{contract.metadata.providerBusinessName}</p>
              )}
            </div>
          </div>
          {contract?.artistEmail && (
            <p className="text-xs text-gray-600 mb-1"><span className="font-medium">Email:</span> {contract.artistEmail}</p>
          )}
          {contract?.artistWhatsapp && (
            <p className="text-xs text-gray-600"><span className="font-medium">WhatsApp:</span> {contract.artistWhatsapp}</p>
          )}
        </CardContent>
      </Card>
  ...
  return (
    <div className="h-full min-h-0 overflow-hidden bg-transparent">
      ...
      <ConfirmDialog
        ...
        variant="warning"
      />
      <Dialog open={showMobileProviderDetails} onOpenChange={setShowMobileProviderDetails}>
      ...
      <div className="flex h-[80vh] min-h-0 flex-col overflow-hidden gap-0 lg:mx-auto lg:h-[88vh] lg:max-h-[88vh] lg:w-full lg:max-w-[1380px] lg:flex-row-reverse lg:gap-5 lg:px-6 lg:py-6">
          ...
          <div className="flex h-full min-h-0 min-w-0 flex-1 flex-col overflow-hidden lg:min-w-[420px]">
            <div className="lg:hidden flex items-center justify-between gap-3 bg-white border-b border-gray-100 px-3 py-1.5">
              <button
                onClick={onBack}
                className="flex items-center gap-1.5 text-[10px] font-medium text-[#1B2A47]"
              >
            ...
            <Card className="flex flex-1 min-h-0 flex-col overflow-hidden rounded-none border-white/40 shadow-sm lg:h-full lg:rounded-2xl lg:border lg:border-white/70 lg:bg-white/95 lg:shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
              ...
              {/* Messages */}
              <div className="flex flex-1 flex-col min-h-0 overflow-hidden p-0">
                {shouldShowWarning && (
          ...
          {/* Right: Provider + Service info */}
          {/* Desktop: shown inline */}
          <div className="hidden shrink-0 lg:block lg:w-[320px] lg:max-h-[88vh] lg:overflow-y-auto">
            <div className="space-y-2">
              <div className="mb-1 flex items-center gap-2 px-1 lg:pt-1">
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={onBack}
